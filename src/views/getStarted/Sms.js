...
import { sms_ccode, sms_name, sms_num } from './redux/getStartSlice'

export function Sms() {
    const name_slice = useSelector(state => state.getStartSlice_data.sms_name)
    const num_slice = useSelector(state => state.getStartSlice_data.sms_num)
    ...
    function submitForm() {
        ...
        navigate("/merchant/get-started/sms-verify")
            ...
            <Card className={Width >= 680 ? 'w-75' : 'w-100'}>
                <CardBody className='d-flex flex-column align-items-center '>
                    <div className="w-100 px-3 my-2 d-flex align-items-center justify-content-between ">
                        <Link to='/merchant/get-started/whatsapp' className="fw-bold">Back</Link>
                        <h4 className="text-center fs-3 fw-bold">SMS Setup</h4>
                        <Link to='/merchant/create-campaign' className="fw-bold">Skip</Link>
                    </div>
                    <h4 className='text-center'>Add your sender information</h4>
                    <p className='text-center mb-2'>Choose the sender name and SMS number appears in your subscribes inbox</p>
                    <form className={`d-flex flex-column align-items-center ${Width >= 680 ? 'w-50' : 'w-100 px-2'}`}>
                        <div className="mb-3 w-100">
                            ...
                            <Input
                                ...
                                onChange={(e) => { setname(e.target.value); dispatch(sms_name(e.target.value)) }}
                        ...
                        <div className="mb-3 w-100">
                            ...
                            <select className="form-select m-0 me-1" onChange={ (e)=>{ setCCode(e.target.value); dispatch(sms_ccode(e.target.value))}} style={{ width: "100px", marginRight: "8px" }}>
                            <option value="91">+ 91</option>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => { setnumber(e.target.value); dispatch(sms_num(e.target.value)) }}
...
export function SmsVerify() {
    ...
    const num_slice = useSelector(state => state.getStartSlice_data.sms_num)
    const code_slice = useSelector(state => state.getStartSlice_data.sms_ccode)
    ...
    return (
        <div className=' d-flex flex-column justify-content-center  align-items-center '>
            ...
            <div className="p-2 px-3 d-flex justify-content-between fw-bold">
                <Link to='/merchant/get-started/sms' >Back</Link>
                <Link to='/merchant/create-campaign' >Next</Link>
                </div>
                <CardBody className=' d-flex flex-column align-items-center '>
                    <MdOutlineTextsms size={120} />
                    <h3 className=' fw-bolder mt-1 mb-1'>Check your SMS</h3>
...

                    {
                        ...
                        <div className='text-center d-flex gap-1 mt-1'>
                            ...
                            <h6>or</h6>
                            <Link to='/merchant/get-started/sms' className='text-dark fw-bold cursor-pointer'>Change Number</Link>
                        </div>
                    }
                    {
                        ShowCount &&
